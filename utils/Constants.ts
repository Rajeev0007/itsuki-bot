...
const COIN = '<:itsukiCoin:1549465313572167751>';
...
/**
 * Maps discord.js `PermissionFlagsBits` key names to the labels users see in
 * Discord's UI. Used in error messages so they never show raw API names like
 * "ManageGuildExpressions" — which Top.gg reviewers flagged as "unknown
 * permission or role".
 */
...
/**
 * Converts a raw `PermissionFlagsBits` key (e.g. `'ManageGuildExpressions'`)
 * into its Discord-UI label (e.g. `'Manage Expressions'`).
 *
 * Falls back to inserting spaces before capital letters so even unlisted keys
 * are rendered as readable text rather than PascalCase.
 */
export function friendlyPermission(key: string): string {
  return PERMISSION_LABELS[key] ?? key.replace(/([a-z])([A-Z])/g, '$1 $2');
}
